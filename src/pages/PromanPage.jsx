...
export default function PromanPage({ onBack, techData }) {
  const [jenisLokasi, setJenisLokasi] = useState('ODP');
  ...
  const handleGenerate = () => {
    const text = `🔧 LAPORAN PROMAN ${jenisLokasi}
📅 ${getFormattedDate()}

📍 LOKASI     : ${jenisLokasi} - ${namaLokasi || '-'}
🏠 ALAMAT     : ${alamat || '-'}
  ...
  return (
    <div className="h-full overflow-y-auto" style={{ paddingBottom: '90px' }}>
      ...
      >
        ...
        <div>
          <p className="text-xs font-bold uppercase tracking-widest" style={{ color: '#e07b39' }}>Generate</p>
          <h2 className="text-base font-black" style={{ color: '#e8eaf0' }}>PROMAN</h2>
        </div>
      </div>
      ...
      <div className="px-4 pt-4 space-y-3">
        ...
        <div style={SECTION_STYLE}>
          <label style={LABEL_STYLE}>Jenis Lokasi</label>
          <div className="flex gap-2">
            {['ODP', 'ODC'].map(j => (
              <button
                key={j}
                onClick={() => setJenisLokasi(j)}
                className="flex-1 py-3 rounded-xl text-sm font-black transition-all"
                style={{
                  background: jenisLokasi === j ? '#e07b39' : '#1e2740',
                  color: jenisLokasi === j ? '#fff' : '#8b95a8',
                  border: `1px solid ${jenisLokasi === j ? '#e07b39' : '#2a3347'}`,
                }}
              >
                {j}
              </button>
        ...
        <div style={SECTION_STYLE}>
          <label style={LABEL_STYLE}>Nama {jenisLokasi}</label>
          <input
            type="text"
            value={namaLokasi}
            onChange={e => setNamaLokasi(e.target.value)}
            placeholder={`Nama ${jenisLokasi}...`}
            style={INPUT_STYLE}
          />
        </div>
        <div style={SECTION_STYLE}>
        ...
        <button
          ...
          style={{ background: 'linear-gradient(135deg, #e07b39, #a04a1a)', color: '#fff', border: 'none' }}
        >
          Generate Format
        </button>
